...
CREATE OR REPLACE FUNCTION check_professor_verified()
    RETURNS TRIGGER AS
$$
DECLARE
    prof_status TEXT;
...
-- 랩장 최대 2명 제한
CREATE OR REPLACE FUNCTION check_lab_leader_limit()
    RETURNS TRIGGER AS
$$
DECLARE
    leader_count INTEGER;
BEGIN
    IF NEW.role = 'LAB_LEADER' AND NEW.left_at IS NULL
        AND (
           TG_OP = 'INSERT'
               OR
...
CREATE OR REPLACE FUNCTION check_paper_schedule_type()
    RETURNS TRIGGER AS
$$
DECLARE
    schedule_type TEXT;
BEGIN
    ...
    IF schedule_type != 'CONFERENCE' THEN
