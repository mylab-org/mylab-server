...
export class CreateBoardResponseDto {
  ...
  @ApiProperty({ type: BoardAuthorDto }) // 객체 타입 명시
  @Expose()
  @Type(() => BoardAuthorDto)
  author: BoardAuthorDto;

  @ApiProperty({ type: BoardLabDto, nullable: true }) // Transform되는 필드도 타입을 명시
  @Expose()
  @Transform(({ obj }) => {
  ...
  @ApiProperty({ example: 5, description: '댓글 개수' })
  @Expose()
  @Transform(({ obj }) => {
